...
class InlineScript extends AbstractViewHelper {
    ...
    private function minifyJS() {
        ...
        foreach ($this->scriptfiles as &$file) {
            ...
            if (strpos($src,".min.js") === false) { // it's already minified
                $jsfile = $file['src']; //array_pop($segments); // get the last element (the concrete filename)
    ...
    /**
     * If the cache file for the given controller/action combination already exists
     * @return boolean
     */
    ...
    /**
     *  Get the cached files but leave the CDN files untouched
     */
